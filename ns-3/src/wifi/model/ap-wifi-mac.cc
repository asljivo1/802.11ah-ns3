...
void ApWifiMac::SetTotalStaNum(uint32_t num) {
    ...
    ApWifiMac::Enqueue(Ptr<const Packet> packet, Mac48Address to, Mac48Address from) {
        ...
        if (to.IsBroadcast() || m_stationManager->IsAssociated(to) || to.IsMulticast6()) {
    ...
    ApWifiMac::SendOneBeacon(void) {
        ...
        if (m_s1gSupported) {
            hdr.SetS1gBeacon();
            /*if (InetSocketAddress::IsMatchingType (this->Mac48Address::GetAddress()))
              {*/
                hdr.SetAddr1(Mac48Address::GetBroadcast()); ///ami
             /* }
            else if (Inet6SocketAddress::IsMatchingType (this->Mac48Address::GetAddress()))
              {
    ...
    ApWifiMac::Receive(Ptr<Packet> packet, const WifiMacHeader *hdr) {
        ...
        if (hdr->IsData()) {
            ...
            if (hdr->IsProbeReq()) {
                NS_ASSERT(hdr->GetAddr1().IsBroadcast() || hdr->GetAddr1().IsMulticast6());
                SendProbeResp(from);
                return;
